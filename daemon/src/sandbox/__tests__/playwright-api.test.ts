...
describe.sequential("QuickJS Playwright Page API coverage", () => {
  ...
  beforeAll(async () => {
    ...
    browserRootDir = await mkdtemp(path.join(os.tmpdir(), "web-interact-playwright-api-"));
  ...
  describe.sequential("navigation", () => {
    ...
    it("supports goBack(), goForward(), and reload()", async () => {
      ...
      const result = await harness.runJson<{
        ...
        // Chromium can restore history entries without firing load/domcontentloaded again.
  ...
  describe.sequential("AI snapshots", () => {
    ...
    it("supports locator.ariaSnapshot()", async () => {
      const result = await harness.runJson<string>(
        withTestPage(
          "snapshot-main",
          `
  ...
  describe.sequential("events", () => {
    ...
    it("supports page.consoleMessages() and page.pageErrors()", async () => {
      const result = await harness.runJson<{
        messages: Array<{ type: string; text: string }>;
        errors: string[];
      }>(
        withTestPage(
          "page-inspection-events",
          `
          await page.evaluate(() => {
            console.info("inspection-log", 456);
            setTimeout(() => {
      ...
      expect(result.messages).toEqual(
        expect.arrayContaining([
          {
            type: "info",
            text: "inspection-log 456",
          },
        ])
      );
      expect(result.errors).toContain("page-boom");
    });

    it("supports page.requests()", async () => {
      const firstUrl = `${navigationServer.baseUrl}/nav/first`;
      const result = await harness.runJson<{
        requestUrls: string[];
      }>(`
        const page = await browser.getPage("page-request-log");
        await page.goto(${JSON.stringify(firstUrl)}, { waitUntil: "load" });
        const requestUrls = (await page.requests())
          .map((request) => request.url())
          .filter((url) => url.includes("/nav/"));
        console.log(JSON.stringify({ requestUrls }));
      `);

      expect(result.requestUrls).toContain(firstUrl);
    });
  });
});
