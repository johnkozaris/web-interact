...
describe.sequential("QuickJS sandbox security", () => {
  ...
  beforeAll(async () => {
    browserRootDir = await mkdtemp(path.join(os.tmpdir(), "web-interact-quickjs-security-"));
  ...
  it("blocks unsafe dynamic imports", async () => {
    await expectSandboxScriptToThrow(
      `await import("node:fs");`,
      /relative imports|module specifier|not supported/i
    );
    await expectSandboxScriptToThrow(
      `await import("patchright");`,
      /bare module specifiers|module specifier|required|not supported/i
    );
  });

  it("allows dynamic imports from the controlled temp directory", async () => {
    const output = await runSandboxScript(`
      ...
      const math = await import("./modules/math.mjs");
      console.log(JSON.stringify({ sum: math.add(2, 3), defaultValue: math.default }));
    `);
  ...
  it("only exposes the expected globals and browser API", async () => {
    ...
    expect(payload.globals).toContain("deleteFile");
    expect(payload.globals).toContain("readFile");
    expect(payload.globals).toContain("resolveFilePath");
    ...
    expect(payload.browserKeys).toEqual([
      "check", "checkElement", "click", "clickAt", "clickElement",
      "closePage", "discover", "drag", "fillElement",
      "getPage", "getPageElements", "listPages", "newPage",
      "select", "selectOption", "type", "waitForSettled"
    ]);
