...
function createManager(
  ...
  const manager = new BrowserManager(path.join("/tmp", "web-interact-auto-connect-tests"), {
...
describe("BrowserManager auto-connect", () => {
  ...
  it("autoConnect falls back from DevToolsActivePort to port probing when the direct websocket is stale", async () => {
    ...
    expect(connectOverCDP).toHaveBeenNthCalledWith(1, staleEndpoint, { isLocal: true });
    expect(connectOverCDP).toHaveBeenNthCalledWith(2, discoveredEndpoint, { isLocal: true });
    expect(requests).toEqual([
      "http://127.0.0.1:9222/json/version",
      "http://127.0.0.1:9223/json/version",
    ]);
