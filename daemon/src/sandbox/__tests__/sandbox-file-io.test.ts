...
describe.sequential("QuickJS sandbox file I/O", () => {
  ...
  beforeAll(async () => {
    ...
    browserRootDir = await mkdtemp(path.join(os.tmpdir(), "web-interact-quickjs-file-io-"));
  ...
  it("saves screenshot buffers into the controlled temp directory", async () => {
    ...
    const expectedPath = await resolveWebInteractTempPath(requestedName);
    ...
    expect(result.savedPath.startsWith(`${path.resolve(WEB_INTERACT_TMP_DIR)}${path.sep}`)).toBe(
      true
    );
  ...
  it("rejects symlinked temp-file targets", async () => {
    ...
    const symlinkPath = path.join(WEB_INTERACT_TMP_DIR, symlinkName);
