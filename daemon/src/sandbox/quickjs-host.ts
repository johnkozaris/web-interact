...
export interface QuickJSModuleLoader {
  loadModule: (moduleName: string) => string;
  normalizeModule?: (baseModuleName: string, requestedName: string) => string;
}
...
export interface QuickJSHostOptions {
  ...
  cpuTimeoutMs?: number;
  moduleLoader?: QuickJSModuleLoader;
...
export class QuickJSHost {
  ...
  readonly #activeAsyncExecutions = new Set<Promise<unknown>>();
  ...
  private constructor(quickjs: QuickJSWASMModule, options: QuickJSHostOptions) {
    ...
    if (options.moduleLoader) {
      this.#runtime.setModuleLoader(
        ...
        options.moduleLoader.normalizeModule
          ? (baseModuleName, requestedName) =>
              options.moduleLoader!.normalizeModule!(baseModuleName, requestedName)
          : undefined
      );
    }
  ...
  async #trackAsyncExecution<T>(execution: Promise<T>): Promise<T> {
  ...
  #toError(_prefix: string, errorHandle: QuickJSHandle): Error {
  ...
  async #drainDisposeCleanup(): Promise<void> {
