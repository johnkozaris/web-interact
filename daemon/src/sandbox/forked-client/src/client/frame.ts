...
import { readTempFile } from "./fileUtils";
...
export class Frame extends ChannelOwner<channels.FrameChannel> implements api.Frame {
  ...
  constructor(
    ...
    type: string,
    guid: string,
    initializer: channels.FrameInitializer
  ) {
  ...
  async waitForNavigation(
    ...
    return await this._page!._wrapApiCall(
      async () => {
        ...
        const navigatedEvent = await waiter.waitForEvent<channels.FrameNavigatedEvent>(
          this._eventEmitter,
          "navigated",
          (event) => {
        ...
        const request = navigatedEvent.newDocument
          ? network.Request.fromNullable(navigatedEvent.newDocument.request)
          : null;
        const response = request
          ? await waiter.waitForPromise(request._finalRequest()._internalResponse())
          : null;
        waiter.dispose();
        return response;
      },
  ...
  async _evaluateExposeUtilityScript<R, Arg>(
    pageFunction: structs.PageFunction<Arg, R>,
    arg?: Arg
  ): Promise<R> {
    assertMaxArguments(arguments.length, 2);
  ...
  async dispatchEvent(
    ...
    type: string,
    eventInit?: any,
    options: channels.FrameDispatchEventOptions & TimeoutOptions = {}
  ): Promise<void> {
  ...
  async $$(selector: string): Promise<ElementHandle<SVGElement | HTMLElement>[]> {
    ...
    return result.elements.map(
      (e) => ElementHandle.from(e) as ElementHandle<SVGElement | HTMLElement>
    );
  }
  ...
  async addScriptTag(
    ...
    const copy = { ...options };
    if (copy.path) {
      copy.content = await readTempFile(copy.path);
  ...
  async textContent(
    ...
    const value = (
      await this._channel.textContent({ selector, ...options, timeout: this._timeout(options) })
    ).value;
  ...
  async innerText(
    ...
    return (
      await this._channel.innerText({ selector, ...options, timeout: this._timeout(options) })
    ).value;
  }
  ...
  async innerHTML(
    ...
    return (
      await this._channel.innerHTML({ selector, ...options, timeout: this._timeout(options) })
    ).value;
  }
  ...
  async getAttribute(
    ...
    const value = (
      await this._channel.getAttribute({
        selector,
        name,
        ...options,
        timeout: this._timeout(options),
  ...
  async inputValue(
    ...
    return (
      await this._channel.inputValue({ selector, ...options, timeout: this._timeout(options) })
    ).value;
  }
  ...
  async isChecked(
    ...
    return (
      await this._channel.isChecked({ selector, ...options, timeout: this._timeout(options) })
    ).value;
  }
  ...
  async isDisabled(
    ...
    return (
      await this._channel.isDisabled({ selector, ...options, timeout: this._timeout(options) })
    ).value;
  }
  ...
  async isEditable(
    ...
    return (
      await this._channel.isEditable({ selector, ...options, timeout: this._timeout(options) })
    ).value;
  }
  ...
  async isEnabled(
    ...
    return (
      await this._channel.isEnabled({ selector, ...options, timeout: this._timeout(options) })
    ).value;
  }
  ...
  async selectOption(
    ...
    values:
      ...
      | SelectOption[]
      | null,
    options: SelectOptionOptions & StrictOptions = {}
  ): Promise<string[]> {
    return (
      await this._channel.selectOption({
        selector,
  ...
  async type(
    selector: string,
    text: string,
    options: channels.FrameTypeOptions & TimeoutOptions = {}
  ) {
  ...
  async _expect(
    expression: string,
    options: Omit<channels.FrameExpectParams, "expression">
  ): Promise<{
    matches: boolean;
    received?: any;
    log?: string[];
...
export function verifyLoadState(name: string, waitUntil: LifecycleEvent): LifecycleEvent {
  if ((waitUntil as unknown) === "networkidle0") waitUntil = "networkidle";
